...
const Navbar = () => {
  ...
  return (
    <nav className="bg-[#e6eef8] sticky top-0 z-50 px-4 py-3 sm:px-6 shadow-md border-b border-slate-200/50">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        {/* Brand Logo */}
        <div className="flex items-center space-x-2.5 cursor-pointer" onClick={() => navigate('/dashboard')}>
          <div className="w-10 h-10 rounded-xl bg-[#9BBEED] flex items-center justify-center text-white font-extrabold text-xl shadow-md shadow-blue-500/5">
            C
          </div>
          <div>
            ...
            return (
              <Link
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-xl text-xs font-bold transition-all duration-200 ${
                  isActive(item.path)
                    ? 'neu-inset text-[#5c81ae]'
                    : 'text-slate-600 hover:bg-[#d8e4f5] hover:text-slate-800'
                }`}
              >
                <Icon size={14} className={isActive(item.path) ? 'text-[#5c81ae]' : 'text-slate-500'} />
